  ...
  while (currentTime <= monthEnd) {
    ...
    for (const trade of intervalTrades) {
      ...
      const swap = parseFloat(trade.swap.replace(/[^\d.-]/g, '') || '0');
        ...
        const netProfit = numericProfit - numericCommission - numericSwap;
        console.log(`Net Profit calculation - Trade ${trade.deal}: profit=${numericProfit} - commission=${numericCommission} - swap=${numericSwap} = ${netProfit}`);
